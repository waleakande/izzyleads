import jwt from 'express-jwt';
import config from 'config';
import attachCurrentUser from './attachCurrentUser';

/**
 * We are assuming that the JWT will come in a header with the form
 *
 * Authorization: Bearer ${JWT}
 *
 * But it could come in a query parameter with the name that you want like
 * GET https://my-bulletproof-api.com/stats?apiKey=${JWT}
 * Luckily this API follow _common sense_ ergo a _good design_ and don't allow that ugly stuff
 */
const getTokenFromHeader = req => {
  /**
   * @TODO Edge and Internet Explorer do some weird things with the headers
   * So I believe that this should handle more 'edge' cases ;)
   */
  if (
    (req.headers.authorization &&
      req.headers.authorization.split(' ')[0] === 'Token') ||
    (req.headers.authorization &&
      req.headers.authorization.split(' ')[0] === 'Bearer')
  ) {
    return req.headers.authorization.split(' ')[1];
  }
  return null;
};

const isAuth = jwt({
  secret: config.get('jwtSecret'), // The _secret_ to sign the JWTs
  userProperty: 'token', // Use req.token to store the JWT
  getToken: getTokenFromHeader, // How to extract the JWT from the request
});

// Only runs the middleware if there is a token in the header
const conditionallyRunMiddleware = async (req, res, next) => {
  const token = getTokenFromHeader(req);
  if (!!token) {
    await isAuth(req, res, next);
  } else {
    // Move to the next middleware if there is no token in the header
    next();
  }
};

const runMiddleware = (required = true) => {
  let middleware = (req, res, next) => next();
  if (required) {
    // Ensure that the authenticate middleware is ran irrespective of the circumstance
    middleware = isAuth;
  } else {
    // Conditionally run the middleware, checking if token is set in the header before moving to the next middleware
    middleware = conditionallyRunMiddleware;
  }
  return async function(req, res, next) {
    const _next = async () => {
      return await attachCurrentUser(req, res, next);
    };
    return await middleware(req, res, _next);
  };
};

export default runMiddleware;
